...
describe("Form.setValues", () => {
	test("By default form's values must be empty", () => {
		const form = new Form();
	...
	test("Execute setValues must insert data inside form.values", () => {
		const form = new Form();
		form.setValues({
			username: "Je-Ne-Si-Us"
		})
		expect(form.values).toEqual({
			username: "Je-Ne-Si-Us",
		})
	})
	test("Values should be parsed by grand-object function.", () => {
		const form = new Form();
		form.setValues({
			"address.city": "Berlin"
		})
	...
	test("Using multiple form.setValues should extend form.values.", () => {
		const form = new Form();
	...
	test("Setting values must override form's values.", () => {
		const form = new Form();
		form.setValues({
			address: { city: "Berlin" }
		})
	...
	test("Form's values must be equal to provided values inside form.setValues.", () => {
		const form = new Form();
		form.setValues({
			name: 'Jack',
			address: {
	...
	test("Inserting empty object should has effect.", () => {
		const form = new Form();
	...
	test("Provided value must rewrite null by composite object.", () => {
		const form = new Form();
	...
	/**
	 * @description Новое значение является примитивом, в отличие от предшествующего значения. То есть сложные объект
	 * заменяется на примитив.
	 */
	test("Primitive value must override the composite value.", () => {
		const form = new Form();
	...
	/**
	 * ОЧЕНЬ ВАЖНО ЭТО ЗАДОКУМЕНТИРОВАТЬ ПОТОМ!
	 *
	 */
	test("Merging values should not override previous values.", () => {
		const form = new Form();
		form.setValues({
			address: {
				test: "Up"
		...
		expect(form.values).toEqual({address: { test: "Up", pest: "Down" }})
	})
	test("Merging empty object should not override previous values.", () => {
		const form = new Form();
		form.setValues({
			address: {
				city: {
	...
	/**
	 * @description При установке поля CoolName, поле Cool не должно как-то влиять на поле CoolName. В данном примере
	 * поле Id и IdentityDocument не должны конфликтовать друг с другом.
	 */
